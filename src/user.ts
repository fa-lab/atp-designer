...
function copy_block() {
  ...
  for (let i = 0; i < table_selected.length; i++) {
    ...
    table_copied.push([
      row,
      col,
      table_selected[i].getAttribute('led_color'),
      table_selected[i].getAttribute('led_bright')
    ]);
  }
...
function move_block(row_offset, col_offset) {
  ...
  if (table_selected.length == 0) return;
  let targets = [];
  for (let i = 0; i < table_selected.length; i++) {
    let row = parseInt(table_selected[i].getAttribute('row'));
    let col = parseInt(table_selected[i].getAttribute('col'));
  ...
  for (let i = 0; i < targets.length; i++) {
    ...
    if (row >= 0 && row < LED_VIEW_SIZE && col < TIMELINE_SIZE * TIME_DIV && col >= 0) {
      ...
      table_elements[row][col].className = (
        table_elements[row][col].className.replace('selected', '') + ' selected'
      ).trim();
    }
  }
  syncData();
  ...
  switch (evt.keyCode) {
    case 36: // Home
      if (TIMELINE_OFFSET == 0) {
        ...
        TIMELINE_OFFSET -= 5;
        if (TIMELINE_OFFSET < 0) TIMELINE_OFFSET = 0;
        update_table();
        evt.preventDefault();
      }
      break;
