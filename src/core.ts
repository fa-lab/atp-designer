...
if (localStorage['atp_led_view_size']) LED_VIEW_SIZE = parseInt(localStorage['atp_led_view_size']);

if (localStorage['atp_timeline_size']) TIMELINE_SIZE = parseInt(localStorage['atp_timeline_size']);

if (localStorage['atp_led_max_size']) LED_MAX_SIZE = parseInt(localStorage['atp_led_max_size']);

if (localStorage['atp_led_channels']) LED_CHANNELS = parseInt(localStorage['atp_led_channels']);
...
function load_project(jsonTxT) {
  ...
  if (fj_result.led_view_size) localStorage['atp_led_view_size'] = parseInt(fj_result.led_view_size);

  if (fj_result.timeline_size) localStorage['atp_timeline_size'] = parseInt(fj_result.timeline_size);

  if (fj_result.led_max_size) localStorage['atp_led_max_size'] = parseInt(fj_result.led_max_size);

  if (fj_result.led_channels) localStorage['atp_led_channels'] = parseInt(fj_result.led_channels);
  ...
  for (let row = 0; row < LED_VIEW_SIZE; row++) {
    for (let col = 0; col < TIMELINE_SIZE * TIME_DIV; col++) {
      if (table_elements[row][col].hasAttribute('led_color')) {
        if (!last_elems[row]) last_elems[row] = {};

        if (!last_elems[row][col]) last_elems[row][col] = [];

        last_elems[row][col] = [
          table_elements[row][col].getAttribute('led_color'),
          table_elements[row][col].getAttribute('led_bright')
        ];
      }
    }
  }
...
function init_table() {
  ...
  for (let row = 0; row < LED_VIEW_SIZE; row++) {
    ...
    for (let col = 0; col < TIMELINE_SIZE * TIME_DIV; col++) {
      ...
      if (col % TIME_DIV == TIME_DIV - 1) t_cell.className = 'br';
